...
Page({
    ...
    getPrice: function() {
      var o = this, coupon_price = 0,promotion_price=0,total_express_price = 0, t = o.data.mch_list, e = o.data.integral_radio, a = (o.data.integral, 
        0), i = 0, s = {}, n = 0, total = 0;
        for (var r in t) {
            ...
            0 == c.offline && (
              //c.express_price && (p += c.express_price), 
            c.offer_rule && 1 == c.offer_rule.is_allowed && (s = c.offer_rule), 
            1 == c.is_area && (i = 1)), a += parseFloat(p);
          if (c.picker_coupon && c.picker_coupon.sub_price){
          coupon_price = a < 0 ? parseFloat(a + c.picker_coupon.sub_price) : parseFloat(c.picker_coupon.sub_price);
          }
            total_express_price += parseFloat(c.express_price);
            total += parseFloat(c.total_price);
            total += parseFloat(c.express_price);
        }
        //添加促销费
        var promotion = o.data.promotion;
        a -= parseFloat(promotion.promotion_price);
        a = a < 0 ? 0 : a;
        if (o.data.mch_offline){
           a += total_express_price;
           a -= parseFloat(promotion.express_price);
        }else{
          total -= parseFloat(promotion.express_price);
        }
        ...
        a = 0 <= a ? a : 0, o.setData({
            ...
            promotion_express_price: parseFloat(promotion.express_price).toFixed(2),
            promotion_price: parseFloat(promotion.promotion_price).toFixed(2),
        });
    },
    cardDel: function() {
        this.setData({
